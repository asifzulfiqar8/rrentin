...
const AreaMapLocation = ({ location, image, name, areaName, houses }) => {
    ...
    const mapCenter =
        areaName && areaPosition ? areaPosition : position || defaultCenter;
                ...
                {areaName ? (
                            ...
                            >
                                <div className="flex justify-between items-center gap-1 p-2.5" style={{ width: '240px' }}>
                                    ...
                                    <div className=" w-[160px]">
                                        <h4 className='truncate' style={{ margin: '5px 0', fontSize: '1rem' }}>{house.address}</h4>
                                        <p className='truncate' style={{ margin: 0, fontSize: '0.9rem' }}>{house.price}</p>
                                    </div>
                                </div>
                            </Tooltip>
                        </Marker>
                    ...
                    position && (
                        ...
                        >
                            ...
                            >
                                <div className="flex justify-between items-center gap-1 p-2.5" style={{ width: '240px' }}>
                                    <div className="relative h-[50px] w-[50px] rounded-md bg-blue-200 overflow-hidden">
                                        <Image
                                            src={image}
                                            alt={name}
                                            layout="fill"
                                    ...
                                    <div className="w-[100px] bg-red-600"> {/* Fixed width for text container */}
                                        <h4 className="text-[1rem] m-0 truncate">{name}</h4>
                                        <p className="text-[0.9rem] m-0 truncate">{location}</p>
                                    </div>
                                </div>
                            </Tooltip>
                        </Marker>
